...
- ITM-016: (Phase 5 r2, F-4 residual — S4) **CI Python version vs dev.** CI ran the suite on **Python 3.11** while local dev/review ran **3.13.2**; both green on the pinned set, but "green == shipped" was only *proven* on 3.11. **✅ CLOSED — Phase 6 / B5 (D-G):** `.github/workflows/ci.yml` now runs a `strategy.matrix.python-version: ["3.11", "3.13"]` (`fail-fast: false`), so the validated set is confirmed on every interpreter actually used.
...
| 1.6 | 2026-06-10 | Engineering | Phase 6 build: **ITM-015 CLOSED** (B2 — uniform DB-error sanitization + error_id; ADR-012) and **ITM-016 CLOSED** (B5 — CI 3.11+3.13 matrix). |
