...
- ITM-016: (Phase 5 r2, F-4 residual — S4) **CI Python version vs dev.** CI runs the suite on **Python 3.11** while local dev/review ran **3.13.2**; both green on the pinned set, but "green == shipped" is only *proven* on 3.11. Recommend a CI matrix (3.11 + 3.13) so the validated set is confirmed on every interpreter actually used. Low priority.
- ITM-015: (Phase 4 r1 F6 + Phase 5 r1 F-2 *400-path* — S3, **deferred to Phase 7**) **Verbatim driver errors** returned as `detail=str(exc)` from `_run_sql`/`/test-connection`/`/profiles/{id}/test` **and `POST /schemas/introspect`** can leak DSN/host/port/username (never the password). Inert on a local single-user tool; **harden before the networked/multi-tenant Phase 7** alongside CORS/auth ([ITM-009](#)/[RISK-12](risk-register.md)): return a generic "Database error — see server logs" to the client, log detail server-side. Rationale for deferral: not exploitable in the current local posture; all DB-error 400s sanitized **uniformly** in one Phase-7 batch (keeping behaviour consistent across endpoints). *Note: the Phase-5 success-path `warnings[]` leak (F-2 200-path) was NOT deferred — it is fixed (generic message + server log).*
