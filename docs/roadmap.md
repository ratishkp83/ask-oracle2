...
| **P6.5** | **Pre-deployment hardening (carried preconditions)** | 🔄 Review done | Opt-in API-key auth + env-driven CORS (ADR-013), base_url encoding fix, atomic store writes + corrupt-record quarantine (ADR-014), non-DB error surfaces routed. **ITM-009/010/013/014/017 CLOSED; RISK-12 Closed/RISK-16 Mitigating.** Exit gate **r1 = PASS-WITH-FIXES** (no S1/S2); R1–R4 remediated; **242 tests.** Closure pending optional r2 spot-check + push — **gate to Phase 7 / any networked deploy.** |
...
## Current focus
**Phase 6.5 — Pre-Deployment Hardening: exit-gate review r1 = PASS-WITH-FIXES, all findings
remediated (242 tests).** All five carried preconditions closed (ITM-009/010/013/014/017;
RISK-12 Closed, RISK-16 Mitigating); the review's four findings (R1 Unicode SSRF fold, R2
fd-close, R3 blank-CORS fallback, R4 doc — none blocking) are fixed. **Next: close P6.5** —
optionally an owner r2 spot-check of the R1 security fix, then push (also demonstrates the CI
3.11+3.13 matrix). Phase 7 (optional: 23ai/EBS) opens only after P6.5 closes; RISK-04
(live-Oracle pass) stays a separate owner-scheduled pre-GA activity. See
