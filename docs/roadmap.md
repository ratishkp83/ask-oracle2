...
ITM-015 + ITM-016 closed; the validated dependency set was re-pinned to a clean-install-proven
3.13-capable configuration (review F-1/F-2), pushed, and **CI run #7 is green on both 3.11 +
3.13**. Next feature work: **Phase 7 (optional)** — but its hard preconditions (CORS/auth ITM-009, base_url ITM-010, file-store
