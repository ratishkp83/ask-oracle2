...
| **C1** | **Pre-GA consolidation & testing** | 🔄 Discovery | Carries CI-matrix green confirmation (on `9209e3a`), **RISK-04** (live-Oracle + manual UI pass — owner provides instance), and pre-GA cleanups ITM-006 (legacy `connection.json`→encrypted-profile migration), ITM-007 (`use_container_width` deprecation), ITM-008 (optional NL PII scrubbing). Charter opened 2026-06-11; pending owner approval. |
...
## Current focus
**Phase 6.5 CLOSED (2026-06-11)** — exit gate passed (r1 PASS-WITH-FIXES → R1–R4 remediated →
closed on r1, no r2; 242 tests; pushed `2ba0a56..9209e3a`). All carried code preconditions for a
networked/multi-tenant deploy are now cleared. **Next: Round C1 — Pre-GA Consolidation &
Testing** ([charters/round-C1-charter.md](charters/round-C1-charter.md), pending owner approval):
confirm the CI 3.11+3.13 matrix green on the pushed commit, run RISK-04 (live-Oracle + manual UI
pass — owner provides the instance), and clear the pre-GA cleanups ITM-006/007/008. Phase 7
(optional: 23ai/EBS) remains available afterward. See [task-tracker](task-tracker.md).
