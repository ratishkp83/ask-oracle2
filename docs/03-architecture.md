...
| `nl2sql.py` | NL→SQL via `LLMConfig` (per-user provider/model/key; env fallback). |
...
**NL→SQL (propose only):** UI/API → `generate_sql_from_nl(q, schema, llm)` → LLM client (per-user or env) → strip fences → `assert_safe_select` → return SQL. **Never executed automatically.**
